...
$.fn.EZView =  function(collectionName) {
        ...
        if (!$('#EZView' + collectionName).length) {
            ...
            var tools = 
                '<div class="tools-container">' +
                    ...
                    '<spam class="tools">' +
                        '<a class="download EZViewHighlight" target="_blank">' +
                        '<img src="./img/icon-img/expand.png" style="width: 50px" title="Download" width="10" /></a>' +
    ...
    self.buildHtmlContent = function() {
        ...
        if (isPdf) {
            htmlContent = '<iframe class="content" frameborder="0" index-content="' + collectionName + imgIndex +
                '" height="' + $(window).height() * 1.50 + '" width="' + $(window).width() * 0.50 +
    ...
    self.setStyles = function() {
        ...
        .find('::-webkit-scrollbar').css({
            'width' : '10px'
        }).end()

            .find('::-webkit-scrollbar-track').css({
            'background' : '#f1f1f1'
        }).end()

            .find('::-webkit-scrollbar-thumb').css({
            'background' : '#888'
        }).end()
            .find('::-webkit-scrollbar-thumb:hover').css({
            'background' : '#555'
        }).end()
